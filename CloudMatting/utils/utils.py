...
import argparse
from .path_utils import get_filename
...
def save_model(model,model_path,epoch,global_step,prefix='model',scope=None):
    ...
    result_dict['gobal_step']=global_step

    model_file=os.path.join(model_path,'%s_%d.pth'%(prefix,epoch))
    torch.save(result_dict,model_file)
...
def vis_cloud_matting(img:torch.Tensor,
                      alpha:torch.Tensor,
    ...
    if len(reflectance.shape)==4:
        if reflectance.shape[1]==1:
            reflectance = np.tile(reflectance[0],(3,1,1))
        else:
            reflectance=reflectance[0]
    else:
        if reflectance.shape[0] == 1:
            reflectance = np.tile(reflectance, (3, 1, 1))
    reflectance = np.transpose(reflectance, (1, 2, 0))
    # reflectance = cv2.resize(reflectance, (img_size, img_size))
    reflectance = reflectance * 255
    reflectance=np.clip(reflectance,0,255)
    reflectance = reflectance.astype(np.uint8)

    result_file=os.path.join(result_path,'{0}_img.png'.format(file_name))
    cv2.imwrite(result_file,img)
    result_file = os.path.join(result_path, '{0}_alpha.png'.format(file_name))
    cv2.imwrite(result_file, alpha)
    result_file = os.path.join(result_path, '{0}_reflectance.png'.format(file_name))
    cv2.imwrite(result_file, reflectance)

    if trimap is not None:
        # trimap=torch.argmax(trimap,dim=1,keepdim=True)
        trimap=trimap.cpu().detach().numpy()
        trimap=trimap*127
        trimap=trimap.astype(np.uint8)
        if len(trimap.shape) == 4:
            if trimap.shape[1] == 1:
                trimap = np.squeeze(trimap[0],axis=0)
            else:
                trimap = trimap[0]
        else:
            if trimap.shape[0] == 1:
                trimap = np.squeeze(trimap,axis=0)
        result_file = os.path.join(result_path, '{0}_trimap.png'.format(file_name))
        cv2.imwrite(result_file, trimap)

def vis_cloud_generation(img:torch.Tensor,
                      alpha:torch.Tensor,
                      reflectance:torch.Tensor,
                      result_path,file_name
                      ):
...
def vis_test_cloud_matting(img:torch.Tensor,
                           alpha:torch.Tensor,
                           reflectance:torch.Tensor,
                           img_files,
                           result_path,trimap:torch.Tensor=None):

    for i in range(len(img_files)):
        img_file=img_files[i]
        file_name=img_file
        if trimap is not None:
            vis_cloud_matting(img[i],alpha[i],reflectance[i],result_path,file_name,trimap=trimap[i])
        else:
            vis_cloud_matting(img[i],alpha[i],reflectance[i],result_path,file_name)
